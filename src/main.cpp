...
int Transformer::number = 1204;
int Transformer::gridfourdetect_threshold = 66;

void setting(Point2f &goal, float &distance, bool &find_object, Mat frame)
{
    goal.x = 0; goal.y = 0;
    distance = frame.cols * frame.rows / (IMAGE_SIZE_COL*IMAGE_SIZE_ROW);
    find_object = false;
}

int main()
{
    //VideoCapture capture("/home/gogojjh/QT/mapRecognition/data/test1.avi");
    ...
    while (1)
    {
        ...
        switch (choice)
        {
            case 0: setting(goal, distance, find_object, frame); trans.detectApriltag(goal, distance, find_object); break; // er wei ma
            case 1: setting(goal, distance, find_object, frame); trans.detectTwo(goal, distance, find_object); break; // jiu gong ge
            case 2: setting(goal, distance, find_object, frame); trans.detectThree(goal, distance, find_object); break; // he ma bian kuang
            case 3: setting(goal, distance, find_object, frame); trans.detectFour(goal, distance, find_object); break; // led \ biankuang
            case 4: setting(goal, distance, find_object, frame); trans.detectRed(goal, distance, find_object); break;
            default:break;
        }
